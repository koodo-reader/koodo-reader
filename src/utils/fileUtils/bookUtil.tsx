...
class BookUtil {
      ...
      return new Promise<void>((resolve, reject) => {
        ...
        reader.onload = async (event) => {
          ...
          try {
            ...
            fs.writeFileSync(path.join(dataPath, 'book', key), Buffer.from(event.target.result as any))
            resolve()
          } catch (error) {
            reject()
            throw error
    ...
    if (isElectron) {
      ...
      return new Promise<void>((resolve, reject) => {
        try {
          ...
          // remove cover
          fs_extra.remove(path.join(dataPath, `book`, key, '.jpg'), (err) => {
            if (err) throw err
            resolve()
          })
        } catch (e) {
  ...
  static isBookExist(key: string, bookPath: string = '') {
    return new Promise<boolean>((resolve, reject) => {
      if (isElectron) {
        ...
        } else if ((bookPath && fs.existsSync(bookPath)) || fs.existsSync(_bookPath)) {
          resolve(true)
        } else {
          resolve(false)
        }
  ...
  static async RedirectBook(book: BookModel, t: (string) => string, history: any) {
    ...
    if (StorageUtil.getReaderConfig('isOpenInMain') === 'yes') {
      history.push(BookUtil.getBookUrl(book) + `?title=${book.name}&file=${book.key}`)
      return
    }
    ...
    if (isElectron) {
      ...
      ipcRenderer.invoke('open-book', {
        url: `${window.location.href.split('#')[0]}#/${ref}/${book.key}?title=${book.name}&file=${book.key}`,
        isMergeWord: book.format === 'PDF' || book.format === 'DJVU' ? 'no' : StorageUtil.getReaderConfig('isMergeWord'),
    ...
    if (isElectron) {
      ...
      let pdfLocation =
        document.URL.indexOf('localhost') > -1
          ? 'http://localhost:3000/'
          : `file://${path.join(__dirname, './build', 'lib', 'pdf', 'web', 'viewer.html')}`
      let url = `${window.navigator.platform.indexOf('Win') > -1 ? 'lib/pdf/web/' : 'lib\\pdf\\web\\'}viewer.html?file=${book.key}`
      return document.URL.indexOf('localhost') > -1 ? pdfLocation + url : `${pdfLocation}?file=${book.key}`
    } else {
  ...
  static getRendtion = (result: ArrayBuffer, format: string, readerMode: string, charset: string) => {
    let rendition
    if (format === 'CACHE') {
      ...
      rendition = new window.Kookit.ComicRender(copyArrayBuffer(result), readerMode, format)
    }
    return rendition
  }
  static generateBook(bookName: string, extension: string, md5: string, size: number, path: string, file_content: ArrayBuffer) {
    return new Promise<BookModel | string>(async (resolve, reject) => {
      let cover: any = ''
      let key: string, name: string, author: string, publisher: string, description: string, charset: string, page: number
      ;[name, author, description, publisher, charset, page] = [bookName, 'Unknown Author', '', '', '', 0]
      let metadata: any
      let rendition = BookUtil.getRendtion(file_content, extension.toUpperCase(), '', '')
      ...
      if (isElectron && cover.indexOf('image') > -1) {
        cover = BookUtil.localizationCover(cover, key)
      }
      ...
      resolve(new BookModel(key, name, author, description, md5, cover, format, publisher, size, page, path, charset))
    })
  }
  static localizationCover(base64String: string, fileName: string) {
    var fs_extra = window.require('fs-extra')
    var path = window.require('path')
    const dataPath = localStorage.getItem('storageLocation')
      ? localStorage.getItem('storageLocation')
      : window.require('electron').ipcRenderer.sendSync('storage-location', 'ping')
    ...
    // 生成完整的文件路径
    const fullFilePath = path.join(dataPath, 'book', fileName)
    fs_extra.writeFile(fullFilePath, Buffer.from(base64String.split(',')[1], 'base64'), (err) => {
      if (err) throw err
      console.log(`${fileName}已经被保存！`)
    })
    return fullFilePath
  }
}
